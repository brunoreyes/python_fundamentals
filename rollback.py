...
class Account(object):
    ...
    def __init__(self, name: str, opening_balance: int = 0):
        ...
        if row: # if row is true if the row isn't 0 or an empty list, dictionary or ins't None
        ...
        self.show_balance()  # want to show balance regardless of if or else statement
    ...
    def deposit(self, amount: int) -> float:
        if amount > 0.0:
            ...
            # new_balance = self._balance + amount
            # deposit_time = Account._current_time.utc.localize(datetime.datetime.utcnow())
            # db.execute("UPDATE accounts SET balance = ? WHERE (name = ?)", (new_balance, self.name))
            # db.execute("INSERT INTO history VALUES(?, ?, ?)", (deposit_time, self.name, amount))
            # db.commit()
            # self._balance = new_balance
            self._save_update(amount) # calling the ._save_update() method and passing it the amount
            print("{:.2f} deposited".format(amount / 100))
        return self._balance / 100
    
    def withdrawl(self, amount: int) -> float:
...
if __name__ == '__main__':
    john = Account("John")
    john.deposit(1010) # I've changed the input to accept cents instead of dollars
    john.deposit(10)
    john.deposit(30)
    john.deposit(10)
    john.withdrawl(0)
