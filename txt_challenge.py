...
with open("sample_python_text_file_in_folder.txt", 'a') as tables:  # a - appends to the end of file, r - reads, w - write a new file
    for i in range(2, 13):
        for j in range(1, 13):
            print("{1:>2} times {0} is {2}".format(i, j, i * j), file=tables) # {1:>2} checks if j > i
        print("-" * 20, file=tables)  # printing a line between each times table