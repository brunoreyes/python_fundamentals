...
class DocType(Tag):
    ...
    def __init__(self):
        super().__init__('!DOCTYPE HTML Public "-//W3C//DTD HTML 4.01//EN" http://www.w3.org/TR/html4/strict.did','')
...
class HtmlDoc(object): # this class is completely comprised of other classes

    def __init__(self, doc_type, head, body):
        self._doc_type = doc_type # with underscores, these fields should be treated as private
        self._head = head
        self._body = body
...
if __name__ == '__main__':
    # my_page = HtmlDoc('title: Demo HTML Document')
    ...
    new_body.add_tag('p', 'The composed object doesn''t actually own the objects that it''s composed of'
                    'If the aggregated object is destroyed, the objects it was comprised of still exist')

    new_docType = DocType()
    new_header = Head("Aggregation Document")
    my_page = HtmlDoc(new_docType, new_header, new_body)

# giving our document new content by switching it's body
